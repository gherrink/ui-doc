...
export type EventListener<M extends EventMap, K extends keyof M> = K extends keyof M
  ? M[K] extends never
    ? () => void
    : M[K] extends unknown[]
      ? (...args: M[K]) => void
      : never
  : never
export type EventArgs<M extends EventMap, K extends keyof M> = K extends keyof M
  ? M[K] extends never
    ? never[]
    : M[K] extends unknown[]
      ? M[K]
...
export type EventListenersMap<M extends EventMap, K extends keyof M = keyof M> = Map<
  K,
  EventListener<M, K>[]
>
